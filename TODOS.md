...
待办事项，按优先级排列。由 /plan-eng-review + /plan-devex-review 于 2026-05-31 生成，2026-06-04 更新。

---

## P0 — 项目骨架（阻塞所有后续开发）

- [x] **项目骨架搭建：Docker Compose + Makefile + 版本锁定 + .env 自动生成**
  - 文件：Makefile, .env.example, .python-version, .nvmrc, docker-compose.yml, Dockerfile
  - 来源：/plan-devex-review DX1

- [x] **README：3步快速启动 + 项目简介 + ASCII 架构图**
  - 来源：/plan-devex-review DX2

- [x] **预填 30 只沪深300 种子数据（2年日线），docker compose up 自动加载**
...
- [x] **标准化 ErrorResponse schema + FastAPI 全局 exception handlers**
  - { error: { code, message, detail } }，前端只需处理一种错误格式
  - 来源：/plan-devex-review DX4

- [x] **请求 correlation_id 中间件 + structlog 自动注入**
  - grep correlation_id 即可追踪完整调用链
  - 来源：/plan-devex-review DX5

- [x] **文档体系：架构文档 + 形态开发指南 + 回测指南 + 故障排查 + CHANGELOG**
...
- [x] **Makefile 封装 Alembic：make migrate / make rollback / make reset-db**
